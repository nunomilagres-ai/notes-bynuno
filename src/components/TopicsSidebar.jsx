...
import { Plus, Pencil, Trash2, StickyNote, Bell } from 'lucide-react'
...
export default function TopicsSidebar({ topics, setTopics, selectedTopic, setSelectedTopic, showReminders, onSelectReminders, pendingReminders }) {
  ...
  return (
    <aside className="flex flex-col h-full" style={{background:'#FFFCF8'}}>
      ...
      <div className="flex-1 overflow-y-auto py-1.5 px-2 flex flex-col gap-0.5">
        <N label="Todas as notas" icon={<StickyNote size={12}/>} active={!showReminders&&selectedTopic===null} onClick={()=>{setSelectedTopic(null);onSelectReminders(false)}}/>
        <N label="Lembretes" icon={<Bell size={12}/>} badge={pendingReminders||undefined} active={showReminders} onClick={()=>onSelectReminders(true)}/>
